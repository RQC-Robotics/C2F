...
def _get_action(
    ...
    quat = utils.normalize_quaternion(R.from_rotvec(obs_tp1['arm/ActualTCPPose'][3:]).as_quat())
    if quat[-1] < 0:
        quat = -quat
    disc_rot = utils.quaternion_to_discrete_euler(quat, rotation_resolution)
    # disc_rot = obs_tp1['arm/ActualTCPPose'][-3:] 

    assert len(bounds_offset) == len(voxel_sizes) -1

    # attention_coordinate = obs_tp1.gripper_pose[:3]
    attention_coordinate = obs_tp1['arm/ActualTCPPose'][:3]
    ...
    for depth, vox_size in enumerate(voxel_sizes):
        ...
        index = utils.point_to_voxel_index(
            obs_tp1['arm/ActualTCPPose'][:3], vox_size, bounds)
    ...
    grip = float((obs_tp1['gripper/pose'] > 0.7))
    
    # rot_and_grip_indicies.extend([int(obs_tp1.gripper_open)])
    rot_and_grip_indicies.extend([int(obs_tp1['gripper/pose'] > 0.7)])

    # return trans_indicies, rot_and_grip_indicies, np.concatenate(
    #     [obs_tp1.gripper_pose, np.array([grip])]), attention_coordinates
    return trans_indicies, rot_and_grip_indicies, np.concatenate(
        [obs_tp1['arm/ActualTCPPose'][:3], R.from_rotvec(obs_tp1['arm/ActualTCPPose'][3:]).as_quat(), np.array([grip])]), attention_coordinates

    #TODO Strange return - concatination not the same
...
def _add_keypoints_to_replay(
    ...
    for k, keypoint in enumerate(episode_keypoints):
        ...
        for name in cameras:
            ...
            #misc - это матрицы камеры* надо добавить в obs ПОДУМАТЬ, МБ СПИЗДИТЬ ИЗ obs_dict
